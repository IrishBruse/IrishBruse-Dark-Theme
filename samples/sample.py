...
# https://github.com/tsoding/porth from here
    ...
    while ip < len(program.ops):
        ...
        try:
            if op.typ in [OpType.PUSH_INT, OpType.PUSH_BOOL, OpType.PUSH_PTR]:
                ...
                assert len(Intrinsic) == 46, "Exhaustive handling of intrinsic in simulate_little_endian_linux()"
...
def type_check_program(program: Program, proc_contracts: Dict[OpAddr, Contract]):
    ...
    while len(contexts) > 0:
        ...
        assert len(OpType) == 18, "Exhaustive ops handling in type_check_program(): len(OpType)=%d"% len(OpType)
        if op.typ == OpType.PUSH_INT:
            ...
            if op.operand == Intrinsic.PLUS:
                ...
                type_check_contracts(op.token, ctx, Contract(ins=[("a", op.token.loc), ("b", op.token.loc), ("c", op.token.loc)], outs=[("a", op.token.loc), ("b", op.token.loc), ("c", op.token.loc), ("a", op.token.loc)]))
            elif op.operand == Intrinsic.ROT:
...
assert len(Intrinsic) == len(INTRINSIC_BY_NAMES), "Exhaustive INTRINSIC_BY_NAMES definition"
...
def usage(compiler_name: str):
    ...
    print("    -I <path>             Add the path to the include search list. The environment variable PORTH_PATH is read if it exists.")
...
if __name__ == '__main__' and '__file__' in globals():
    ...
    if os.environ.get('PORTH_PATH') is not None:
        include_paths.extend(os.environ['PORTH_PATH'].split(':'))
